...
import (
	...
	"lxxccc.work/LToolkits/configTools/jsonConfig"
	"lxxccc.work/Ltoolkits/netTools"
...
func main() {
	...
	jsonConfig.ReadJSONConfigToStruct("config.json", config)

	//获取公网IP信息
	publicIP, err := netTools.GetPublicIPUseDnspod()
	...
	for _, record := range oldRecord.DomainRecords.Record {
		if record.DomainName == config.DomainName && record.RR == config.RR {
			if record.Value == publicIP {
				log.Printf("当前配置解析地址与公网IP相同，不需要修改。")
				return
			}
	...
	if 0 < len(exsitRecordID) {
		...
		if nil != err {
			log.Println("修改解析失败", err)
		} else {
			log.Println("修改解析成功", rsp)
		}
	} else {
		//没有找到配置记录，那么就新增一个
		...
		if nil != err {
			log.Println("添加DNS解析失败", err)
		} else {
			log.Println("添加DNS解析成功", rsp)
		}
	}
}
